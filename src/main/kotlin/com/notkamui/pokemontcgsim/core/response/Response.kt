...
open class Response<out T>(
    ...
    companion object {
        fun <S> success(status: HttpStatusCode, result: S? = null) = Response(status, success = ResultData(result))
        fun error(status: HttpStatusCode, error: MessageDTO<out DTO>? = null) = Response<Nothing>(status, error = ResultData(error))
        fun error(status: HttpStatusCode, error: MessageKeyDTO) = error(status, MessageDTO.simple(error))

        fun ok() = success(HttpStatusCode.OK, null)
