...
open class ActionResponse<out T : DTO>(
    ...
    companion object {
        fun <T : DTO> success(
            status: HttpStatusCode,
            result: MessageDTO<T>? = null
        ): ActionResponse<T> = ActionResponse(status, success = ResultData(result))

        fun error(
            status: HttpStatusCode,
            result: MessageDTO<out DTO>? = null
        ): ActionResponse<Nothing> = ActionResponse(status, error = ResultData(result))
